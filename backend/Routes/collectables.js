...
router.route("/updatecollectable/:collectableid").put(async (req, res) => {
  ...
  const updatecollectable = await Collectable.findByIdAndUpdate(
    userID,
    updateCollectable
  )
    .then(() => {
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .send({
          status: "Error with updating collectable",
          error: err.message,
        });
    });
});
...
router.route("/deletecollectable/:collectableid").delete(async (req, res) => {
  ...
  await Collectable.findByIdAndDelete(userID)
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .send({ status: "Error with delete collectable", error: err.message });
    });
});
