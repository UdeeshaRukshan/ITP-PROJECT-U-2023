...
router.route("/updateart/:artid").put(async (req, res) => {
  let userID = req.params.artid;
  const { title, medium, height, width, condition, location, value, images } =
    req.body;
  ...
  const updateart = await Art.findByIdAndUpdate(userID, updateArt)
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .send({ status: "Error with updating art", error: err.message });
    });
});
...
router.route("/deleteart/:artid").delete(async (req, res) => {
  ...
  await Art.findByIdAndDelete(userID)
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .send({ status: "Error with delete art", error: err.message });
    });
});
