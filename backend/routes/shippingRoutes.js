...
router.route("/").get((req,res)=>{
  ...
  Shipping.find().then((shippingRoutes)=>{
...
router.route("/update/:id").put(async (req,res) => {
  ...
  const {firstName, lastName, addressLine1, addressLine2, city, province, postalCode} = req.body;
...
router.route("/get/:id").get(async (req, res) => {
  ...
  .then((shipping) => {
      ...
      console.log(err.message);
      res.status(500).send({status: "Error with get shipping details", error: err.message});
  })
})
